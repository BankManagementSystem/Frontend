<script>
	...
	async function handleLoans(event) {
		event.preventDefault();
		goto('/Loans');
	}
	async function handleLockers(event) {
		event.preventDefault();
		goto('/Lockers');
	}
...
<div class="flex flex-col bg-[#772035] h-[20vh] text-[#FDFDFD]">
	...
	>
		<div><button class={isActive('/Home') ? 'active' : ''} on:click={handleHome}>Home</button></div>
		<div>
			<button class={isActive('/Accounts') ? 'active' : ''} on:click={handleAccounts}
				>Accounts</button
			>
		</div>
		<div>
			<button class={isActive('/transfer') ? 'active' : ''} on:click={handleTransfer}
				>Transfer</button
			>
		</div>
		<div>
			<button class={isActive('/Cards') ? 'active' : ''} on:click={handleCards}>Cards</button>
		</div>
		<div>
			<button class={isActive('/Loans') ? 'active' : ''} on:click={handleLoans}>Loans</button>
		</div>
		<div>
			<button class={isActive('/Lockers') ? 'active' : ''} on:click={handleLockers}>Lockers</button>
		</div>
	</div>
</div>
