	...
	<div class="flex flex-col bg-[#772035] h-[20vh] text-[#FDFDFD]">
		...
		>
			<div>
				<div><button class={isActive('/employeesHome') ? 'active' : ''} on:click={handleHome}>Home</button></div>
			</div>
			<div>
				<button class={isActive('/ELoanApplications') ? 'active' : ''} on:click={handleLoan}>Loan Applications</button>
			</div>
			<div>
				<button class={isActive('/EBranches') ? 'active' : ''} on:click={handleBranches}>Branches</button>
			</div>
		</div>
		{#if showPopup}
